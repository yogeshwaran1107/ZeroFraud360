"""Training Pipeline for Real-Time Banking Fraud Detection using LightGBM.

Features:
1. Ingests raw data or falls back to realistic synthetic banking data.
2. Stratified 80/20 train/validation split to preserve fraud minority distribution.
3. Class imbalance mitigation using scale_pos_weight calculation.
4. Early stopping on the validation set using Average Precision (PR-AUC).
5. Comprehensive evaluation (PR-AUC, ROC-AUC, Precision, Recall, Decision Thresholds).
6. Persists the production booster (`fraud_model.txt`) and feature metadata (`features.json`).
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train LightGBM Banking Fraud Detection Model."
    )
    parser.add_argument(
        "--data-path",
        type=str,
        default=None,
        help="Path to CSV dataset. If omitted or not found, generates synthetic data.",
    )
    parser.add_argument(
        "--artifacts-dir",
        type=str,
...
def run_training(
    ...
    """Executes the full model training and artifact export workflow.
    ...
    logger.info("=== STEP 3: Initializing & Training LightGBM ===")
    ...
    logger.info(
        "MFA Trigger (>=0.30):   Precision: %.4f | Recall: %.4f",
        precision_mfa,
        recall_mfa,
    )
    logger.info(
    ...
    model_txt_path = artifacts_dir / "fraud_model.txt"
    ...
    schema_metadata = {
        ...
        "model_architecture": "LightGBM Booster",
